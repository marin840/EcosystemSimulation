...
namespace AnimalBehaviour
{
    public class AnimalStateManager : MonoBehaviour
    {
        ...
        [FormerlySerializedAs("foodMask")] [Header("Layer masks")] 
        public LayerMask foodLayer;
        public LayerMask borderLayer, decorationLayer, waterMeshLayer, friendsLayer, foxLayer;
        ...
        [Header("Animal preferences")] public float speedPref;
        ...
        public AnimalAbstractState FoodSearchState = new FoodSearchState();
        public AnimalAbstractState WaterSearchState = new WaterSearchState();
        public AnimalAbstractState MateSearchState = new MateSearchState();
        public AnimalAbstractState FleeState = new RabbitFleeState();
        private void Awake()
        {
        ...
        void FixedUpdate()
        {
            ...
            if (Physics.SphereCast(_rigidbody.position - movementDirection * 0.1f, 0.05f, movementDirection, out hit, 0.4f, decorationLayer)
                || Physics.SphereCast(_rigidbody.position, 0.05f, movementDirection, out hit, 0.2f, friendsLayer))
            {
                if (!hasJumped)
                {
                    hasJumped = true;
            ...
            if(!isStopped && movementDirection.magnitude > 0.1f)
        ...
        {
            float speedDif = Math.Abs(speedPref - animal.AnimalSpeed) / speedPref;
            float senseDif = Math.Abs(sensePref - animal.SenseRange) / sensePref;
            float sizeDif = Math.Abs(sizePref - animal._transform.localScale.x) / sizePref;
        ...
        private void Update()
        {
            ...
            if (hunger >= 1 || thirst >= 1)
            {
                ...
                {
                    FoxManager.Instance.FoxDies(gameObject, s);
                    _animator.Play("Fox_Death");
                }
        ...
        private AnimalAbstractState CalculatePriorityState()
        {
            ...
            if (CompareTag("Rabbit"))
            {
                if (CheckNearbyPredators() && hunger < 0.9 && thirst < 0.9)
                {
                    return FleeState;
                }
            }
        ...
        private bool CheckNearbyPredators()
        {
            Collider[] foxColliders = Physics.OverlapSphere(_rigidbody.position, SenseRange * 0.5f, foxLayer);
            return foxColliders.Length > 0;
        }
        ...
        private IEnumerator DevelopAnimal()
        {
        ...
        public void RabbitKilled(GameObject rabbit)
        {
            Destroy(rabbit);
        }
    }
